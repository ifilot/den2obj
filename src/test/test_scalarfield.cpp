...
void TestScalarField::testReading() {
    ...
    CPPUNIT_ASSERT_DOUBLES_EQUAL( (double)-0.276298 / V, sf.get_min(), 1e-8 );
    CPPUNIT_ASSERT_DOUBLES_EQUAL( (double)2047.064424 / V, sf.get_max(), 1e-4 );

    // read specific values from the scalar field
    CPPUNIT_ASSERT_DOUBLES_EQUAL( sf.get_grid_ptr()[0], sf.get_value_interp(0.0,0.0,0.0), 1e-8 );
    CPPUNIT_ASSERT_DOUBLES_EQUAL( sf.get_grid_ptr()[1], sf.get_value_interp(0.1,0.0,0.0), 1e-8 );
    CPPUNIT_ASSERT_DOUBLES_EQUAL( sf.get_grid_ptr()[100], sf.get_value_interp(0.0,0.1,0.0), 1e-8 );
    CPPUNIT_ASSERT_DOUBLES_EQUAL( sf.get_grid_ptr()[100*100], sf.get_value_interp(0.0,0.0,0.1), 1e-8 );

    // also test some false values
    CPPUNIT_ASSERT( std::abs(sf.get_grid_ptr()[50*100*100 + 50 * 100 + 49] - sf.get_value_interp(5.0,5.0,5.0)) > 1e-8 );
    // CPPUNIT_ASSERT_DOUBLES_EQUAL( sf.get_grid_ptr()[50*100*100 + 50 * 100 + 50], sf.get_value_interp(5.0,5.0,5.0), 1e-8 );
    CPPUNIT_ASSERT( std::abs(sf.get_grid_ptr()[50*100*100 + 50 * 100 + 51] - sf.get_value_interp(5.0,5.0,5.0)) > 1e-8 );
